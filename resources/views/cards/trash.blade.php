                ...
                <div class="col">
                    <div class="card h-100 shadow-sm border-danger">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mb-3">
                                <p class="mb-1">
                                    <strong><i class="fas fa-car me-1"></i>Марка:</strong> {{ $card->brand }}
                                </p>
                                <p class="mb-1">
                                    <strong><i class="fas fa-cog me-1"></i>Модель:</strong> {{ $card->model }}
                                </p>
                                <p class="mb-0">
                                    <strong><i class="fas fa-calendar me-1"></i>Год:</strong> {{ $card->year }}
                                </p>
                            </div>
                            ...
                            <div class="d-flex gap-2 mt-auto">
                                ...
                                <form action="{{ route('cards.restore', $card->id) }}" 
                                      method="POST" 
                                      class="flex-grow-1"
                                      onsubmit="return confirm('Восстановить машину \"{{ $card->title }}\" и вернуть в каталог?')">
                                    @csrf
                                ...
                                <form action="{{ route('cards.forceDelete', $card->id) }}" 
                                      method="POST" 
                                      onsubmit="return confirm('УДАЛИТЬ НАВСЕГДА машину \"{{ $card->title }}\"?\n\nЭто действие нельзя отменить!')">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="alert alert-info mt-4">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    ...
                    В корзине: <strong>{{ $cards->count() }}</strong> автомобилей
                </div>
